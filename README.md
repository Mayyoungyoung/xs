...
本轮新增测试：共创协议纯函数（选区锚点、采纳模式、重复采纳、受控剧情操作、锁定、线程与上下文分项）、协作状态持久化与迁移（含损坏数据与旧备份）、泳道布局纯函数（确定性、阶段分栏、分级显示、聚焦/折叠/搜索/定位），以及 DOM 级的共创流程（讨论不写入、候选稿未确认不落盘、生成期间修改原文被拒绝、锁定阻止采纳、迟到响应绑定原章节、剧情候选校验与整组采纳、取消生成不留残稿）。真实模型质量和账户连通性需要配置密钥后测试。
